...
export const MarketCard = ({ market, type }: Props) => {
	...
	const contractList = contracts?.map((contract: RecordType) => (
		<div
			key={contract.id}
			className='flex gap-10 justify-evenly items-center py-5 border-solid border-b-zinc-600 border-b-[1px]'>
	...
	return (
		<>
			<li className='list-none flex flex-col gap-1 p-5 bg-zinc-200 dark:bg-zinc-900 rounded-sm'>
				<h2 className='font-bold text-xl pb-3 border-solid border-b-zinc-600 border-b-[1px]'>
					{market.name}
				</h2>
				{contractList}
			</li>
